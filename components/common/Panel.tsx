...
export default function Panel() {
  ...
  return (
    <div
      className={`text-gray-500 z-30 right-0 flex top-0 bottom-0 flex-col fixed w-full sm:w-1/2 transition-all duration-300 ${nav.active ? 'translate-x-0  ' : 'translate-x-full'}  bg-gray-800`}
    >
      <ul className="flex flex-col text-xl xl:text-3xl 2xl:text-4xl">
        ...
        <li className="flex">
          <Link href="/contacto">
            ...
            >
              {state.content.panel.contact}
            </a>
          </Link>
        </li>
        <li className="pl-6 my-4 2xl:my-8 flex">
          <p className="xl:text-xl 2xl:text-2xl text-base mr-3 ">
            {state.content.panel.theme}
          </p>
          <button
            onClick={changeTheme}
            className="bg-gray-300  shadow-inner  p-0.5 rounded-3xl w-12 xl:w-16 flex items-center  "
          >
            <span className="flex shadow-2xl items-center text-base translate-x-0 dark:translate-x-7 xl:dark:translate-x-9 transition-all duration-300 ease-out  rounded-full p-2 xl:p-3 bg-gray-800 text-gray-300 justify-center"></span>
          </button>
        </li>
